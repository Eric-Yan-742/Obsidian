[[CS/服务器/TCP-IP 网络编程 笔记/TCP-IP 网络编程 笔记|TCP-IP 网络编程 笔记]]
...
- 五种I/O模型
    - UNP Section 6.2, p154.
        ...
        ![[_attachments/Untitled 213.png|Untitled 213.png]]
...
- 服务器框架：I/O处理单元是服务器管理客户连接的模块。它通常要完成以下工作：等待并接受新的客户连接，接收客户数据，将服务器响应数据返回给客户端。但是，数据的收发不一定在I/O处理单元中执行，也可能在逻辑单元中执行，具体在何处执行取决于事件处理模式
    ...
    ![[_attachments/Untitled 1 62.png|Untitled 1 62.png]]
    
- 事件处理模式：Reactor/Proactor
    
    ![[_attachments/Untitled 2 39.png|Untitled 2 39.png]]
    
    ![[_attachments/Untitled 3 27.png|Untitled 3 27.png]]
    
    ![[_attachments/Untitled 4 20.png|Untitled 4 20.png]]
...
- 并发模式：同步/异步（和I/O模式的同步异步不是一个东西）
    ...
    - 图示
        ...
        ![[_attachments/Untitled 5 18.png|Untitled 5 18.png]]
...
- 半同步/半异步模式：同步线程用于处理客户逻辑，相当于图8-4中的逻辑单元；异步线程用于处理I/O事件，相当于图8-4中的I/O处理单元 。异步线程监听到客户请求后，就将其封装成请求对象并插入请求队列中。请求队列将通知某个工作在同步模式的工作线程来读取并处理该请求对象。
    ...
    ![[_attachments/Untitled 6 18.png|Untitled 6 18.png]]
    
- 半同步/半反应堆模式
    
    ![[_attachments/Untitled 7 16.png|Untitled 7 16.png]]
...
- 图示
    ...
    ![[_attachments/Untitled 8 16.png|Untitled 8 16.png]]
...
- 条件变量
    ...
    [[CS/服务器/pthread_cond_t|pthread_cond_t]]
