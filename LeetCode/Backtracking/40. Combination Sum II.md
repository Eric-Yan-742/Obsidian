...
- Remove same values in the same level. Don’t remove same values in the same branch
    ...
    One branch will never explore the same combination  by incrementing `startIdx`. But two branches may have overlapping combinations. That’s why **later elements with the same values in the same level** will cause repetition.
