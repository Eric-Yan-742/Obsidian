...
- Use `totalSum` to determine the values to return:
	1. `curSum` can help us exclude positions that are not possible. However, that doesn't mean values that are not excluded buy `curSum` 
    2. If totalSum > 0, there must exist a unique valid starting point, at least the last point has positive curSum.
