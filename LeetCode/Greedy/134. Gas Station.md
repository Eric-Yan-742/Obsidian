...
- Use `totalSum` to determine the values to return:
	...
	2. If `totalSum < 0` , there cannot be valid positions. If `totalSum >= 0` , there must be valid positions, which means interval that is not excluded by `curSum` must contain the valid position. 
