...
3. Why we don't need to consider the case of deleting 2 chars from both strings? `dp[i - 1][j - 1] + 2 >= dp[i][j - 1] + 1` and `dp[i - 1][j - 1] + 2 >= dp[i - 1][j] + 1` because  `dp[i][j - 1]` and `dp[`
